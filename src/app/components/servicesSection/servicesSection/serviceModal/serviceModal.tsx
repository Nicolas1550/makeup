...
const ServiceModal: React.FC<ServiceModalProps> = ({
  ...
  const [selectedAvailabilityId, setSelectedAvailabilityId] = useState<
    number | null
  >(null); // Nuevo estado
  const [confirmReservationForHelperOpen, setConfirmReservationForHelperOpen] =
    useState(false);
  ...
  const handleSelectAvailability = (availabilityId: number) => {
  ...
  const calendarEvents = [
    ...availabilities.map((availability) => ({
      id: availability.id,
      title: availability.estado === "reservado" ? "Reservado" : "Disponible",
      start: new Date(availability.fecha_inicio),
      end: new Date(availability.fecha_fin),
      allDay: false,
      estado: availability.estado || "disponible", // Asegúrate de que esta línea esté presente
    })),
    ...selectedEvents,
  ];
  ...
  const saveDescriptionChanges = async () => {
    try {
      ...
      alert("Hubo un error al actualizar la descripción. Inténtalo de nuevo.");
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off("serviceDescriptionChanged", handleServiceDescriptionChanged);
    };
            ...
            >
              ...
              <ServiceModalContent>
                ...
                {isUserAssigned && (
                  <ActionContainer>
                    {isEditingOptions ? (
                      <ActionButton onClick={() => setIsEditingOptions(false)}>
                    ...
                    {isEditingCarousel ? (
                      <ActionButton onClick={() => setIsEditingCarousel(false)}>
                        Finalizar Edición del Carrusel
                      </ActionButton>
                    ) : (
                      <ActionButton onClick={() => setIsEditingCarousel(true)}>
                        Editar Carrusel
                      </ActionButton>
                    )}
