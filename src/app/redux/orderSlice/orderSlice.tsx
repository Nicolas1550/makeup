  ...
  async ({ orderId, newState }, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `https://sofiacomar1.latincloud.app/api/orders/update-status/${orderId}`,
        {
          method: "PUT",
          headers: {
...
>("order/fetchUserOrders", async (_, { rejectWithValue }) => {
  ...
  try {
    const response = await fetch(
      "https://sofiacomar1.latincloud.app/api/orders/user-orders",
      {
        headers: {
          "x-auth-token": userToken,
        },
...
>(
  ...
  async ({ status, sortByDate, startDate, endDate }, { rejectWithValue }) => {
    ...
    let url = `https://sofiacomar1.latincloud.app/api/orders/orders-by-status/${status}`;
    const queryParams = [];
    if (sortByDate) {
...
>("order/deleteOrderById", async (orderId, { rejectWithValue }) => {
  ...
  try {
    const response = await fetch(
      `https://sofiacomar1.latincloud.app/api/orders/order/${orderId}`,
      {
        method: "DELETE",
        headers: {
          "x-auth-token": userToken,
